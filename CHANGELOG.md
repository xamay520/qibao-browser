...
## v0.1.5 — 移除站点导览（2026-09-12）

- **移除 qibao.online 站点导览**：该功能会让首次访问 `qibao.online` 时弹出 5 步引导层，且底层网页未加载导致引导下方为黑屏，体验差。用户需求是单独做视频介绍网站、顺带提及浏览器，不需要浏览器内嵌导览。现已整体移除（HTML overlay 容器、renderer 导览逻辑、CSS 样式全部删除），访问 `qibao.online` 直接加载网站本身。
- 保留 v0.1.4 的地址栏可见/可输入、弹窗拦截、导航协议补全等能力。
